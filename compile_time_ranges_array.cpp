...
{
    return std::tuple_cat(make_pairs_tuple(first, second), 
                          make_pairs_tuple(args...));
}
...
constexpr auto make_pairs_array()
{
    ...
    return tuple_to_array<std::tuple_element_t<0, decltype(pairsTuple)>,
                          tupleSize>(pairsTuple, 
...
constexpr auto make_ranges_array()
{
    ...
    std::array<decltype(pairsArr[0].first),
               calc_ranges(T, Y, Args...)> rangesArr{};
    std::size_t i = 0;
